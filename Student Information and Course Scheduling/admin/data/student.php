...
{
  $sql = "SELECT s.*, st.strand_name as strands,
           COALESCE(s.lrn, LPAD(CAST(s.student_id AS CHAR), 12, '0')) as formatted_lrn 
           FROM students s
           LEFT JOIN strands st ON s.department = st.strand_code
...
function getStudentByLRN($lrn, $conn)
{
  $sql = "SELECT * FROM students
           WHERE lrn=?";
  $stmt = $conn->prepare($sql);
