...
function saveEditedComment(commentId, newContent) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (response.success) {
        ...
        commentElement
          .querySelectorAll("button.save-button, button.cancel-button")
  ...
  xhr.send(
    "comment_id=" + commentId + "&content=" + encodeURIComponent(newContent)
  );
}
...
function saveEditedReply(replyId, newContent) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (response.success) {
        ...
        replyElement
          .querySelectorAll(
            "button.save-reply-button, button.cancel-reply-button"
          )
          .forEach((btn) => btn.remove());
  ...
  xhr.send(
    "reply_id=" + replyId + "&content=" + encodeURIComponent(newContent)
  );
}
...
function cancelEditReply(replyId, originalContent) {
  ...
  // Remove the Save and Cancel buttons
  replyElement
    .querySelectorAll("button.save-reply-button, button.cancel-reply-button")
    .forEach((btn) => btn.remove());
