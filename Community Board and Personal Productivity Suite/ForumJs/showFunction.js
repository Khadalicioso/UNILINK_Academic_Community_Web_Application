document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadState(postId) {
    const commentsExpanded = localStorage.getItem(
      `comments-expanded-${postId}`
    );

    // Restore comments state
    if (commentsExpanded === "true") {
      document
        .querySelectorAll(`#comments-${postId} .comment`)
      ...
      document.querySelector(
        `.view-less-comments[data-post-id='${postId}']`
      ).style.display = "inline-block";
      document.querySelector(
        `.show-more-comments[data-post-id='${postId}']`
      ).style.display = "none";
    } else {
    ...
    document
      .querySelectorAll(`#comments-${postId} .replies`)
      .forEach((repliesSection) => {
        const commentId = repliesSection.getAttribute("id").split("-")[1];
        const repliesExpanded = localStorage.getItem(
          `replies-expanded-${commentId}`
        );
        if (repliesExpanded === "true") {
          document
            .querySelectorAll(`#replies-${commentId} .reply`)
            .forEach((reply, index) => {
              if (index >= 1) {
          ...
          document.querySelector(
            `.view-less-replies[data-comment-id='${commentId}']`
          ).style.display = "inline-block";
          document.querySelector(
            `.show-more-replies[data-comment-id='${commentId}']`
          ).style.display = "none";
        }
      });
  }
