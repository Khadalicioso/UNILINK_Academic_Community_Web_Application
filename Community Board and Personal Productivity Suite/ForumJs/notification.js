document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchNotifications() {
    fetch("Forum_action.php?fetch_notifications=true")
      ...
      .then((data) => {
        ...
        if (data.likes && data.likes.length) {
          ...
          data.likes.forEach((like) => {
            notificationHtml += `
                            <div class="notification-item" data-post-id="${
                              like.post_id
                            }">
                                <p>Post: "${like.post_content}" received ${
              like.like_count
            } new like(s).</p>
                                ${
                                  like.post_image
                                    ? `<img src="${like.post_image}" alt="Post Image" style="width: 50px; height: 50px; object-fit: cover;">`
                                    : ""
                                }
                                <a href="post-details.php?id=${
                                  like.post_id
                                }" class="view-post-link">View Post</a>
                            </div>
                        `;
          });
        }
            ...
            notificationHtml += `
                            <div class="notification-item" data-post-id="${
                              comment.post_id
                                ...
                                <p>Post: "${comment.post_content}" received ${
              comment.comment_count
            } new comment(s).</p>
                                ${
                                  comment.post_image
                                    ? `<img src="${comment.post_image}" alt="Post Image" style="width: 50px; height: 50px; object-fit: cover;">`
                                    : ""
                                }
                                <a href="post-details.php?id=${
                                  comment.post_id
      ...
      .catch((error) => {
        console.error(error);
        notificationBody.innerHTML =
          "<p>Error fetching notifications. Please try again later.</p>";
      });
  }

  function updateNotificationCount(count) {
    const lastViewedCount =
      parseInt(localStorage.getItem("lastViewedNotificationCount")) || 0;
  ...
  function markNotificationsAsViewed() {
    const currentCount =
      parseInt(localStorage.getItem("lastFetchedNotificationCount")) || 0;
