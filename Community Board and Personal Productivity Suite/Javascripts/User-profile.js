...
document
  .getElementById("loadPostsButton")
      ...
      .then((data) => {
        ...
        data.forEach((row) => {
          ...
          if (row.post_id !== currentPostId) {
            ...
            postsHtml += `
                        <div class="post-container">
                            ...
                            ${
                              row.post_image
                                ? `<img src="${row.post_image}" alt="Post Image" class="post-image">`
                                : ""
                            }
                            <span class="timestamp">Posted on ${new Date(
                              row.post_created_at
                            ).toLocaleString()}</span>
          ...
          if (row.comment_id) {
            postsHtml += `
                        <div class="comment">
                            <p><strong>${row.commenter_username}:</strong> ${
              row.comment_content
            }</p>
                            <span class="timestamp">Commented on ${new Date(
